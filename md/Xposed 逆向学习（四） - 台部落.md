...
*   本篇的最后要实现的效果就是文字自动回覆。在此基础上你可以去对接图灵机器人来回复，做一个可以陪自己聊天的机器人。
*   首先要实现自动回覆，我们肯定要能够获取接收到的消息。据我所知微信接收到消息是会将记录插入到数据库的，所以我就来源码中通过类搜索查找到SQLiteDatabase，不要问我为什么知道微信数据库类是这个，我没法回答你这个问题，这是经验。
...
*   从源码中可以看到insert 插入数据库的方法最终返回了insertWithOnConflict 方法。所以我们来hook 下这个方法。
