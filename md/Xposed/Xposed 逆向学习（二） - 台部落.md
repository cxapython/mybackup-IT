...
*   本文的最终目的就是逆向查找到微信聊天时发送图片的方法，会写的相对简单，里面会留有很多值得进一步学习查找的提示，希望抱着学习心态来的你可以按照提示去进一步挖掘学习哦。
*   提示一：可以按照类似打招呼那篇走界面控件和点击事件结合源码的方法从选择图片点击发送后一步一步跟着去查找。
*   本文通过的方式是一种倒推参数的方式哦，首先在打招呼的那篇博客里如下图我稍微提了一下微信很多功能都可能封装在了同一个方法里，参数不同实现的功能也就不同了。打招呼和发图两个页面点开来看下会不会感觉特别的相似，既然这么相似我就有理由猜测最终走的是同一个方法哦，也就是那个a 方法了。其实即使页面没啥相似你也可以这样去猜测，做出猜测之后你再去进行合理的的验证，万一验证出来是对也就成功了。这方法怎么感觉有点像上学时的假设检验方法，这应该算学以致用吧，这波联系我觉得也是没谁了。
